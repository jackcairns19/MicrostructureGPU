...
def getMicrostructure(liquidFraction, moleFraction):
	...
	#Initialize grid:
	cp.random.seed(123)
	x = cp.array(np.meshgrid(range(N), range(N))) #grid
	x0 = cp.random.rand(2,nGrains)*N #seed locations for each grain
	rAvg = cp.sqrt(N*N/(cp.pi*nGrains)) #avg grain radius in full solid
	r0 = (cp.random.rand(nGrains)*moleFraction + cp.random.rand(nGrains)*(1-moleFraction))*rAvg #nucleation time, converted to radius using growth rate
	r0 -= r0.min() #Without loss of generality earliest grain nucleates at t=0
	rDiff = cp.linalg.norm(x[:,None]-x0[:,:,None,None], axis=0) + r0[:,None,None] #time at which each grain would reach each pixel (if no other grains were present)
	grainID = cp.argmin(rDiff, axis=0) #determine which grain will get to each pixel first
	rMin = cp.min(rDiff, axis=0) #r at which each pixel would become part of a grain (liquid before)
	...
	#Convert to image
	pixels = cp.tile(solidColor[None,None,:], (N,N,1))
	pixels[cp.where(grainID<0)] = liquidColor
	pixels[edges] = edgeColor
	print(time.time() - t0)
	return Image.fromarray(pixels.get())
